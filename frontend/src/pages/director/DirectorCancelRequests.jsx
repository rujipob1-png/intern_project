...
export default function DirectorCancelRequests() {
  ...
  const loadPendingCancels = async () => {
    try {
      setLoading(true);
      const data = await directorAPI.getPendingCancelRequests();
  ...
  const handleApproveCancel = async (leaveId) => {
    const confirmed = await confirm({
      ...
      message: 'เมื่อกดยืนยัน ใบลานี้จะถูกยกเลิก และวันลาจะคืนกลับให้ผู้ขอลา',
      type: 'warning',
    ...
    try {
      ...
      await directorAPI.approveCancelRequest(leaveId, remarks);
  ...
  const handleRejectCancel = async (leaveId) => {
    ...
    try {
      setActionLoading(true);
      await directorAPI.rejectCancelRequest(leaveId, remarks);
      ...
      <div className="bg-[#1a2744] rounded-xl p-6 text-white">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400 mt-1">ผู้อำนวยการกลุ่มงาน - รออนุมัติยกเลิก</p>
          </div>
      ...
      {pendingCancels.length > 0 && (
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {Object.entries(departmentStats).sort((a, b) => b[1] - a[1]).map(([dept, count]) => (
              <button
                key={dept}
      ...
      {pendingCancels.length === 0 ? (
        ...
        <div className="grid gap-6">
          {filteredCancels.map((leave) => (
            <Card key={leave.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-gray-200">
                ...
                <div className="p-6 lg:flex-1 space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="text-sm font-semibold text-gray-700">เหตุผลที่ขอยกเลิก:</p>
                    </div>
                    <p className="text-gray-800 font-medium">{leave.cancelledReason || 'ไม่ระบุเหตุผล'}</p>
                  </div>

                  {/* Original Reason */}
                  {leave.reason && (
                    <div className="bg-gray-50 rounded-xl p-4">
                      <p className="text-sm font-medium text-gray-600 mb-1">เหตุผลการลาเดิม:</p>
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-2">
                        <button
                          ...
                          className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-all disabled:opacity-50"
                        >
