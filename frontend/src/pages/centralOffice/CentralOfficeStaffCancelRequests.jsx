...
import { XCircle, CheckCircle, Clock, Calendar, FileText, AlertCircle, AlertTriangle, Filter, Building2, Users, ArrowLeft, Search } from 'lucide-react';
...
export default function CentralOfficeStaffCancelRequests() {
  ...
  // Get unique departments from pending cancels
  ...
  const loadPendingCancels = async () => {
    try {
      setLoading(true);
      const data = await centralOfficeAPI.getPendingCancelRequestsStaff();
  ...
  const handleApproveCancel = async (leaveId) => {
    const confirmed = await confirm({
      ...
      message: 'เมื่อกดยืนยัน ใบลานี้จะถูกยกเลิก (จะส่งต่อไปยังหัวหน้าสำนักงานกลาง)',
      type: 'warning',
    ...
    try {
      ...
      await centralOfficeAPI.approveCancelLevel2(leaveId, remarks);
  ...
  const handleRejectCancel = async (leaveId) => {
    ...
    try {
      setActionLoading(true);
      await centralOfficeAPI.rejectCancelLevel2(leaveId, remarks);
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-200">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${selectedDepartment === 'all' ? 'bg-slate-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
            >
            ...
            {Object.entries(departmentStats)
              ...
              .map(([dept, count]) => (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${selectedDepartment === dept ? 'bg-slate-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                    }`}
                >
      ...
      {pendingCancels.length === 0 ? (
        ...
        <div className="grid gap-6">
          {filteredCancels.map((leave) => (
            <Card key={leave.id} className="overflow-hidden hover:shadow-lg transition-shadow duration-300 border-l-4 border-slate-500">
              <div className="flex flex-col lg:flex-row">
                ...
                <div className="p-6 lg:flex-1 space-y-4">
                  ...
                  {leave.directorCancelApproval && (
                    <div className="bg-blue-50 rounded-xl p-4">
                      <p className="text-sm font-medium text-blue-600 mb-1">ผอ.กลุ่มงานอนุมัติยกเลิกแล้ว:</p>
                      <p className="text-blue-800">{leave.directorCancelApproval.approver}</p>
                    </div>
                  )}
                </div>
